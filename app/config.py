...
def merge_cors_origins(allowed_origins: str, frontend_url: str) -> list[str]:
    """Merge ALLOWED_ORIGINS and FRONTEND_URL into a single CORS origins list."""
    # Parse existing allowed_origins
    origins = set(parse_allowed_origins_to_list(allowed_origins))
    
    # Add FRONTEND_URL origins if provided
    if frontend_url:
        frontend_origins = parse_allowed_origins_to_list(frontend_url)
        origins.update(frontend_origins)
...
class Settings(BaseSettings):
    ...
    # Railway-specific FRONTEND_URL for CORS (comma-separated list supported)
    frontend_url: str = Field(default="", description="Frontend URL(s) for CORS, comma-separated")
    ...
    def get_cors_origins(self) -> list[str]:
        """Get merged CORS origins from both ALLOWED_ORIGINS and FRONTEND_URL."""
        return merge_cors_origins(self.allowed_origins, self.frontend_url)
...
def get_settings() -> Settings:
    ...
    if not s.debug and not _skip_public_origin_check:
        ...
        local_origins = []
        for origin in s.get_cors_origins():
            netloc = urlparse(origin).netloc
            if _is_local_netloc(netloc):
                local_origins.append(origin)
        if local_origins:
            raise RuntimeError(
                "ALLOWED_ORIGINS/FRONTEND_URL contains localhost/127.0.0.1 in non-debug mode. "
                "Set origins to public URLs only."
            )
    return s
